...
suppressWarnings({
  ...
  for (fold_idx in 1:k) {
    ...
    model <- lm(bfkg ~ bs(sc, knots=c(quantile(train$sc, probs=c(0.1,0.5,0.9))),degree=3), data = train)
    
    # Generate bootstrap samples and fit models
    bootstrap_models <- lapply(1:B, function(b) {
      bootstrap_indices <- sample(1:nrow(train), replace = TRUE)
      bootstrap_sample <- train[bootstrap_indices, ]
      lm(bfkg ~ bs(sc, knots=c(quantile(bootstrap_sample$sc, probs=c(0.1,0.5,0.9))),degree=3), data = bootstrap_sample)
    })
    # Calculate residuals and r_bar
    x_bar <- mean(train$sc)
    ssx <- sum((train$sc - x_bar)^2)
    h <- (1 / nrow(train)) + ((train$sc - x_bar)^2) / ssx
    r <- residuals(model) / sqrt(1 - h)
    r <- na.omit(r) 
    r_bar <- mean(r)
    
    alpha <- 0.025
    quantile_high <- (B + 1) * alpha
    quantile_low <- (B + 1) * (1 - alpha)
    ...
    for (j in 1:nrow(test)) {
      prediction_errors <- sapply(1:B, function(b) {
        predict(bootstrap_models[[b]], newdata = data.frame(sc = test$sc[j])) - 
          (predict(model, newdata = data.frame(sc = test$sc[j]))+sample(r-r_bar,M))
      })
...
# Combine results into a single data frame for easier analysis
...
ggplot(results, aes(x = sc, y = bfkg)) +
  ...
  labs(title = "Prediction Intervals for leafs_data using nonparametric Bootstrap for Each Fold",
       x = "Predictor (sc)",
       y = "Response (bfkg)") +
  theme_minimal() +
  facet_wrap(~ fold)